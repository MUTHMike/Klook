...
Drupal.behaviors.ckeditor = function(context) {
	...
	$('ul.demoNav li a').click(function() {
		$('ul.demoNav li').each(function() {
...
//		$(document.createElement("button")).text(Drupal.t('View source')).appendTo($('#demoCode')).click(function() {
//			$('#demoCode').empty().hide();
//			$('#demoCode').html('<pre><code class="html">' + editor.getData().replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</code></pre> ').slideDown("slow");
//			$('code.html').chili();
//		});
	...
	$('#demo1', context).click(function() {
		editor = CKEDITOR.appendTo('demoInside', { height: 300 });
		editor.setData(initialHtml);
	});

	$('#demo2', context).click(function() {
		var select = $(document.createElement("select")).attr("id", "languages").change(function() {
			var editorData = editor ? editor.getData() : initialHtml;
			if (editor) {
				editor.destroy();
			}
			editor = CKEDITOR.appendTo('demoInside', { language: $(this).val() });
			editor.setData(editorData);
		}).appendTo($('.demoInside').html('<label for="languages">' + Drupal.t('Select a language:') + '</label>&nbsp;'));

		$('.demoInside').append('<p>(' + Drupal.t('You may see strange characters if your system doesn\'t support the selected language') + ')</p>');

		for (var i = 0; i < window.CKEDITOR_LANGS.length; i++) {
			$("<option>").attr("value", window.CKEDITOR_LANGS[i].code).text(window.CKEDITOR_LANGS[i].name).appendTo(select);
		}

		editor = CKEDITOR.appendTo('demoInside',
			{
				on:
				{
					instanceReady: function() {
						select.val(this.langCode);
					}
				}
			});
	...
	$('#demo3', context).click(function() {
		$('.demoInside').append('<p>The CKEditor toolbar can be fully customized. The following is the sample of the two presets available:<br></p>');

		$('.demoInside').append('<p id="toolbarBasic">Editor with the "Basic" toolbar:<br></p>');
		editor = CKEDITOR.appendTo('toolbarBasic', { toolbar: 'Basic', height: 120 });

		$('.demoInside').append('<p id="toolbarFull">Editor with the "Full" toolbar:<br></p>');
		editor = CKEDITOR.appendTo('toolbarFull', { toolbar: 'Full', height: 120 });
	});

	$('#demo4', context).click(function() {
		$('.demoInside').append('<p id="skinKama">Editor with the "Kama" skin:<br></p>');
		editor = CKEDITOR.appendTo('skinKama', { skin: 'kama', height: 120 });

		$('.demoInside').append('<p id="skinOffice2003">Editor with the "Office 2003" skin:<br></p>');
		editor = CKEDITOR.appendTo('skinOffice2003', { skin: 'office2003', height: 120 });

		$('.demoInside').append('<p id="skinV2">Editor with the "V2" skin (the good old times):<br></p>');
		editor = CKEDITOR.appendTo('skinV2', { skin: 'v2', height: 120 });
	});

	$('#demo_uicolor', context).click(function() {

		html = '<p id="uiColorSpace" style="margin-bottom:40px;">Click one of the colors to change the editor interface:<br>';

		var colors = ['#FFC4C4','#FFAD69','#FFCD69','#FFE569','#FFFF69','#BFEE62','#99F299','#91E6E6','#C2CEEA','#B19FEB','#CD98EA','#F299CC'];

		for (var i = 0; i < colors.length; i++)
		{
			html += '<a style="background-color:' + colors[i] + ';border:1px solid #777E7F;float:left;width:20px;height:20px;margin-right:10px;margin-bottom:10px;display:block;cursor:pointer" onclick="window.demo_editor.setUiColor(\'' + colors[i] + '\')">&nbsp;</a>';
		}
		
		html += '</p>';
		
		$('.demoInside').append( html );

		editor = CKEDITOR.appendTo('demoInside');
		editor.setData(initialHtml);
		
		window.demo_editor = editor;
	});
}
...
$(document).ready(function() {
	var showCompatibilityMsg = function()
	{
		...
		var html = '<p><strong>Your browser is not compatible with CKEditor.</strong></p><p>';
		...
		var browsers =
		{
			...
			opera : 'Opera 9.5',
			webkit : 'Google Chrome and Safari 4.0'
		};
...
$(document).ready(function() {
	$('#renewalOptions').hide();
	$('#torenew').show();
	$('#torenew, #torenewh').bind('click', function(){
		$('#renewalOptions').slideDown('normal');
		$('#torenew').hide();
		return false;
	});
	...
	$('#buydeveloper').hide();
	$('#tobuydeveloper').show();
	$('#tobuydeveloper').bind('click', function(){
		$('#buydeveloper').slideDown('normal');
		$('#tobuydeveloper').hide();
		return false;
	});
	...
	$("a.cksource").tooltip({
		...
		return Drupal.t("During the purchase process you\'ll be redirected to the CKSource website.");
	},
	showURL: false
	});
	...
	var browsers =
	{
		...
		firefox : { Label : 'Firefox 3.0+', OSs : { windows : 1, mac : 1, linux : 1 } },
		safari : { Label : 'Safari', OSs : { windows : 1, mac : 1 } },
		opera : { Label : 'Opera', OSs : { windows : 1, mac : 1, linux : 1 } },
		chrome : { Label : 'Google Chrome', OSs : { windows : 1, mac : 1, linux : 1 } }
	} ;
	...
	$('.btnDemo a, .btnDownload a, .btnLearn a, .btnBuyCKSource a, .btnRenewCKSource a, p.downloads a, .boxProductTour p a img').hover(function() {
		$(this).fadeTo(200, '0.8');
	}, function() {
	...
	$('#a2007').click(function() {
		$('#sp2010').fadeOut('fast', function(){
			$('#sp2007').fadeIn();
			$("#a2007").addClass("sp_selected");
			$("#a2010").removeClass("sp_selected");
		});
		return false;
	});
	$('#a2010').click(function() {
		$('#sp2007').fadeOut('fast', function(){
			$('#sp2010').fadeIn();
			$("#a2010").addClass("sp_selected");
			$("#a2007").removeClass("sp_selected");
		});
		return false;
	});
	...
	if ($.fancybox) {
		...
		$("a[rel=sp2007]").fancybox(opts);
		$("a[rel=sp2010]").fancybox(opts);
		$("a[rel=fancybox]").fancybox(opts);
	}
	...
	$(window).scroll(function() {
		...
		$('#fixedbox')
			.stop()
			.animate({"top": Math.max( $(window).scrollTop()-170, 75 ) + "px"}, "slow" );
	});
});;
